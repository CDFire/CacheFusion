...
def create_similarity_matrix(layer_idx, kv_cache, beta, page_size, num_tokens_to_process):
    """Cluster tokens and calculate similarities for each pair within clusters."""
    ...
    return hybrid_sim, temporal_sim, semantic_sim
...
for layer_idx in layer_indices:
    try:
        ...
        cluster_results = create_similarity_matrix(layer_idx, kv_cache, beta, page_size, num_tokens_to_process)
        h_sim, temp_sim, sem_sim = cluster_results
        output_file = os.path.join(output_dir, f"layer_{layer_idx}_cluster_with_similarities.npz")
